...
const getUsers = async (type: MY_USER_TYPES): Promise<MyUser[]> => {
  return new Promise((resolve, reject) => {
    MyUserModel.find({type}).toArray((error, found) => {
      if (error) {
        console.error(error);
        reject(error);
      } else {
...
const getUser = async (userId): Promise<MyUser> => {
  return new Promise((resolve, reject) => {
    MyUserModel.find({_id: userId}).toArray((error, found) => {
      if (error) {
        console.error(error);
        reject(error);
      } else {
...
const addUser = (type: MY_USER_TYPES, inputs: IgnisignSigner_CreationRequestDto): Promise<MyUser> => {
  ...
  try {
    return new Promise((resolve, reject) => {
      MyUserModel.insert({...inputs, type, signatureProfileId}, async (error, found)=>{
        if (error) {
          reject(error);
        } else {
          try {
            ...
            MyUserModel.update({_id: userId}, {signerId, ignisignAuthSecret: authSecret, ...inputs, type, signatureProfileId}, (error, f)=>{
              resolve(found[0]);
            })
